...
const createRide = async (req, res) => {
  try {
    ...
    // Coordinate-only clients can omit display labels; the route service only
    // needs the coordinates, while the persisted ride still requires labels.
    if (!requestData.pickup && requestData.pickupLatitude !== undefined &&
        requestData.pickupLongitude !== undefined) {
      requestData.pickup = `${requestData.pickupLatitude},${requestData.pickupLongitude}`;
    }

    if (!requestData.destination &&
        requestData.destinationLatitude !== undefined &&
        requestData.destinationLongitude !== undefined) {
      requestData.destination =
        `${requestData.destinationLatitude},${requestData.destinationLongitude}`;
    }

    const validatedData = createRideSchema.parse(requestData);
